...
with c_minus:
    if st.button("−"):
        st.session_state.benches = clamp(st.session_state.benches - 1)
with c_plus:
    if st.button("＋"):
        st.session_state.benches = clamp(st.session_state.benches + 1)
...
with c_slider:
    st.slider(
        "Benches (add/remove)",
        min_value=BMIN, max_value=BMAX, step=1,
        value=int(st.session_state.benches),
...
# ── Interactive diagram (shapes, arrows, x1/x2) ────────────
# Canvas (0..10)
...
# ── KPI + Gauge (kept; title not cut off) ───────────────────
